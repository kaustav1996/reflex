...
### 0.1 Deterministic rules first, on every call (S)
...
### 0.2 An unsure router steps up, never down (S)
...
### 0.3 Pin the Jev version and log it (S)
...
### 0.4 Skill hints: two stages, and weak matches declined (S)
- **Jev asks:** stage 1 is a `Choice` over the full skill list plus yes/no gate questions ("does
  ...
  candidate.
- **Code decides:** suggest a skill only when the gate and the fit both clear the bar (at least
  0.5, today it is 0.3), and only if the returned name is in the local list.
- **From:** jev-agent-skill-router, jev-skill-router (#91), TypeSafe's skill-suggestion cookbook.

### 0.5 A labeled test set for the gate, run in CI (M)
...
### 1.1 Trim tool output before the model reads it (M)
...
### 1.2 Compaction: prune first, summarize only if that isn't enough (M)
...
### 1.3 Send only the tools, skills and connectors the prompt needs (M)
...
### 1.4 Routing that doesn't break the prompt cache (M)
...
### 1.5 Triage failures before the model spends a turn on them (M)
...
### 1.6 Check before waking a scheduled agent (S)
...
### 1.7 Point the agent at the right files (M)
...
### 2.1 A decision log with outcomes (M)
...
### 2.2 Collect outcomes from things Reflex already sees (M)
...
### 2.4 Replay thresholds and get suggestions (M)
...
### 2.5 Shadow mode for every new decision (S)
...
### 2.7 Non-English states need their own thresholds (S)
...
### 3.1 Screen web, browser and connector results for prompt injection (M)
...
### 3.2 A pre-commit and pre-push hook template (S)
...
### 3.3 Check the project's own rules after edits (M)
...
### 3.4 "Is the answer backed by what the tools returned?" (S)
...
### 4.1 A completion check that costs nothing when it isn't needed (S)
...
### 4.2 Answer the agent's open questions when it's safe (M)
...
### 4.3 Browser and computer use (M)
...
### 4.5 A plain-language command palette (S)
...
5. Everything else, in the order the savings report and the calibration page suggest.
