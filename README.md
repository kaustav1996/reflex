...
| Model router and skill/connector selector, together in one request-level call | fast / default / strong tier? which bundled skill, which connected MCP server is relevant? (with "none" escapes) | switches the model and the thinking effort per request when confident, for example a cheap model at low effort for a rename and a strong one at high effort for "why does it crash under load". A model you pick yourself is pinned for the session and routing pauses until `/reflex route auto`; the model picker can also change the three tiers for one session only; and injects a `<relevance>` hint so the model reads the right skill and reaches for the right tools. Both judgments ride in one Jev request: questions are answered in parallel and the state is charged once, so asking them separately cost twice for nothing |
| Screening on results written elsewhere (connectors, `browse`, `browser_read`) | does this text address the reader with instructions? does it ask for secrets or an external endpoint? | a result that asks for secrets or a fetch is prefixed as DATA, not instructions; one that merely speaks to the reader is named as someone else's words. Nothing is removed, and you are told. Off with `screen outside results`. |
