...
| Model router before each request | fast / default / strong tier? | switches the model and the thinking effort per request when confident, for example a cheap model at low effort for a rename and a strong one at high effort for "why does it crash under load". A model you pick yourself is pinned for the session and routing pauses until `/reflex route auto`; the model picker can also change the three tiers for one session only |
